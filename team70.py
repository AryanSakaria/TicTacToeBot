	...
	def mini_max(self, depth, isMax, alpha, beta, board, old_move, playing_bonus, current_move_temp):
		...
		for i in range(len(valid_moves_mini)):
			...
			if isMax:
				...
				if isBonus:
					if playing_bonus:
						value100000 = self.mini_max(depth+1, True, alpha, beta, board_temp, current_move, False, old_move)
					else:
			...
			while x < 3 :
				while y < 3 :
					if board.small_boards_status[k][x][y] == 'o' and x % 3 == 1 and y % 3 == 1 :
						heuristic_value = heuristic_value - small_board_multiplier * 4 * 3
					elif board.small_boards_status[k][x][y] == 'o' and x % 3 != 1 and y % 3 != 1 :
						heuristic_value = heuristic_value - small_board_multiplier * 3 * 4
					elif board.small_boards_status[k][x][y] == 'o' :
						heuristic_value = heuristic_value - small_board_multiplier * 2 * 6
